...
def создать_отзыв(реквест):
    ...
    контекст = {"форма": заполненная_форма}
    # если пользователь попытался внести и зменения в текст, но форма оказалсь невалидна, то в переменой "заполненная_форма" сохранились те изменения, которые он успел внести.
    return render(реквест, "редактировать_отзыв.html", контекст)
def редактировать_отзыв(реквест, айди_отзыва):
    ...
    if реквест.method == "GET":
        ...
        # этот отзыв уже существует, значит в форму нужно заполнить егор данные(текст, картинку, тег)
        контекст = {"форма": форма, "отзыв": отзыв}
        return render(реквест, "редактировать_отзыв.html", контекст)
    
    заполненная_форма = ФормаОтзыва(реквест.POST, реквест.FILES, instance=отзыв)
    if заполненная_форма.is_valid():
        if "картинка" in заполненная_форма.changed_data:
            # если в заполненной форме в списке "изменённая информация" присутствует "картинка", значит старую надо удалить
            ...
            # data - информация
            # date - дата
        заполненная_форма.save()
        return redirect("один_отзыв", айди_отзыва=айди_отзыва)
    
    контекст = {"форма": заполненная_форма, "отзыв": отзыв}
    # если пользователь попытался внести и зменения в текст, но форма оказалсь невалидна, то в переменой "заполненная_форма" сохранились те изменения, которые он успел внести.
    return render(реквест, "редактировать_отзыв.html", контекст)
